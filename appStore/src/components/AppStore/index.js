...
class AppStore extends Component {
  state = {searchInput: '', currentView: tabsList[0].tabId}
  ...
  render() {
    const {searchInput, currentView} = this.state

    const filteredApps = appsList.filter(each => each.category === currentView)
    console.log(filteredApps)
    const finalApps = filteredApps.filter(each =>
          ...
          <ul className="tabs">
            {tabsList.map(eachTab => (
              <TabItem
                ...
                isActive={eachTab.tabId === currentView}
              />
            ))}
          </ul>
